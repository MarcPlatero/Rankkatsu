...
<template>
  <AppLayout>
    <div class="max-w-3xl mx-auto p-6">
      <h1 class="text-2xl font-bold mb-6">⚙️ {{ $t('Configuració') }}</h1>

      <section class="mb-8 p-6 border rounded-lg bg-white dark:bg-gray-800 dark:border-gray-700 shadow">
        <h2 class="text-lg font-semibold mb-3 text-gray-900 dark:text-gray-100">{{ $t('Aparença') }}</h2>
        <div class="flex items-center justify-between">
          <span class="text-gray-700 dark:text-gray-300">
              {{ $t('Mode') }} {{ isDarkMode ? $t('fosc') + ' 🌙' : $t('clar') + ' ☀️' }}
          </span>
          <button
