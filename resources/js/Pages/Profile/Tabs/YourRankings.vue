...
const confirmDelete = (rankingId) => {
  if (confirm(trans('⚠️ Estàs segur que vols eliminar aquest rànquing? Aquesta acció no es podrà desfer.'))) {
...
<template>
  <div>
    <h2 class="text-xl font-semibold mb-4 text-gray-900 dark:text-gray-100">{{ $t('🏆 Els teus rànquings') }}</h2>
    ...
    <p v-else class="text-gray-500 dark:text-gray-400 text-center">
      {{ $t('Encara no has creat cap rànquing.') }}
    </p>
  </div>
</template>